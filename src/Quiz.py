...
# con = MySQLdb.connect("localhost", "root", "root", "regs")
...
def connect():
    ...
    if not validate_email(s8):
        ...
        # Clear the entry field to prompt the user to re-enter the email
...
def open_second_window():
    ...
    canvas1 = Canvas(root, width = 1366, height = 745) 
...
def open_software_test():
    ...
    software_app = SoftwareTest(software_window,t)
def open_tally_test():
    cate.destroy()
    m.withdraw()  
    tally_window = Toplevel(t)
    tally_app = TallyTest(tally_window, t)
def open_gk_test():
    cate.destroy()
    m.withdraw()  
    gk_window = Toplevel(t)
    gk_app = GKTest(gk_window, t)
...
class SoftwareTest:
    def __init__(self, root, main_window):
        ...
        self.root.title("Software Test")
        self.root.protocol("WM_DELETE_WINDOW", self.on_close)
        self.result_shown = False
        self.software_frame = Frame(self.root)
        self.software_frame.pack()

        self.timer_label = Label(self.software_frame, text="", font=("Lucida Console", 12,"bold"), fg="red")
        self.timer_label.pack(pady=10)

        self.question_label = Label(self.software_frame, text="", font=("Comic Sans MS", 20,"bold"), wraplength=350, justify="left")
        self.question_label.pack(pady=20)

        self.answer_var = IntVar()
        self.option_buttons = []

        self.software_questions = [
        ...
        self.correct_answers = [1, 3, 0, 3, 3, 2, 3, 1, 2, 0]
        self.user_answers = []
    ...
    def display_question(self):
        question = self.software_questions[self.current_question_index]
    ...
    def update_options(self):
        ...
        options = self.software_options[self.current_question_index]
        for i, option in enumerate(options):
            option_button = Radiobutton(self.software_frame, text=option, font=("Comic Sans MS",16), variable=self.answer_var, value=i)
        ...
        self.submit_button = Button(self.software_frame, text="Submit",width=8, font=("Comic Sans MS",16), bg="#66E04E", command=self.submit_answer)
    ...
    def submit_answer(self):
        ...
        if len(self.user_answers) < len(self.software_questions):
    ...
    def calculate_score(self):
        # global score
        correct_count = 0
        for user_answer, correct_answer in zip(self.user_answers, self.correct_answers):
            if user_answer == correct_answer:
                correct_count += 1
        total_questions = len(self.software_questions)
    ...
    def show_result(self, score):
        ...
        user = (s1,s3,s9,category,str(score),result)
        cur.execute(sql, user)
        con.commit()
        con.close()
        ...
        play_sound("chipi-chipi-chapa-chapa-made-with-Voicemod.mp3")
        ...
        categoryv = Label(result_window,text=category, font=("copperplate Gothic Bold",22),bg="#CCFF90")
...
class TallyTest:
    def __init__(self, root, main_window):
        ...
        self.root.geometry("500x500")
        self.main_window = main_window
        self.root.title("Accountance Test")
        self.root.protocol("WM_DELETE_WINDOW", self.on_close)
        self.result_shown = False
        self.tally_frame = Frame(self.root)
        self.tally_frame.pack()

        self.timer_label = Label(self.tally_frame, text="", font=("Lucida Console", 12,"bold"), fg="red")
        self.timer_label.pack(pady=10)

        self.question_label = Label(self.tally_frame, text="", font=("Comic Sans MS", 20,"bold"), wraplength=350, justify="left")
        self.question_label.pack(pady=20)

        self.answer_var = IntVar()
        self.option_buttons = []

        self.tally_questions = [
            "1. Good will is",
        ...
        self.correct_answers = [1, 2, 0, 1, 0, 3, 0, 1, 1, 2]
        self.user_answers = []
    ...
    def display_question(self):
        question = self.tally_questions[self.current_question_index]
        self.question_label.config(text=question)
        self.start_timer()
        self.update_options()
    ...
    def update_options(self):
        for option_button in self.option_buttons:
            option_button.destroy()
        self.option_buttons = []
        options = self.tally_options[self.current_question_index]
        for i, option in enumerate(options):
            option_button = Radiobutton(self.tally_frame, text=option, font=("Comic Sans MS",16), variable=self.answer_var, value=i)
            option_button.pack(anchor="w", padx=10)
            self.option_buttons.append(option_button)
        ...
        self.submit_button = Button(self.tally_frame, text="Submit",width=8, font=("Comic Sans MS",16), bg="#66E04E", command=self.submit_answer)
        self.submit_button.pack()
    ...
    def submit_answer(self):
        answer_index = self.answer_var.get()
        self.user_answers.append(answer_index)
        if len(self.user_answers) < len(self.tally_questions):
            self.current_question_index += 1
            self.display_question()
        else:
                self.calculate_score()
    def calculate_score(self):
        ...
        for user_answer, correct_answer in zip(self.user_answers, self.correct_answers):
            if user_answer == correct_answer:
                correct_count += 1
        total_questions = len(self.tally_questions)
        score = (correct_count / total_questions) * 100
        self.show_result(score)
...
class GKTest:
    def __init__(self, root,main_window):
        ...
        self.root.geometry("500x500")
        self.main_window = main_window
        self.root.title("General Test")
        self.root.protocol("WM_DELETE_WINDOW", self.on_close)
        self.result_shown = False
        self.gk_frame = Frame(self.root)
        self.gk_frame.pack()
        self.timer_label = Label(self.gk_frame, text="", font=("Lucida Console", 12,"bold"), fg="red")
        self.timer_label.pack(pady=10)
        self.question_label = Label(self.gk_frame, text="", font=("Comic Sans MS", 20,"bold"), wraplength=350, justify="left")
        self.question_label.pack(pady=20)
        self.answer_var = IntVar()
        self.option_buttons = []
        self.gk_questions = [
        ...
        self.correct_answers = [3, 2, 3, 3, 2, 3, 1, 0, 2, 0]
        self.user_answers = []
        self.current_question_index = 0
        self.remaining_time = 0
        self.timer_id = None
    ...
    def display_question(self):
        question = self.gk_questions[self.current_question_index]
        self.question_label.config(text=question)
        self.start_timer()
        self.update_options()
    ...
    def update_options(self):
        for option_button in self.option_buttons:
            option_button.destroy()
        self.option_buttons = []
        options = self.gk_options[self.current_question_index]
        for i, option in enumerate(options):
            option_button = Radiobutton(self.gk_frame, text=option, font=("Comic Sans MS",16), variable=self.answer_var, value=i)
            option_button.pack(anchor="w", padx=10)
            self.option_buttons.append(option_button)
        if hasattr(self, 'submit_button') and self.submit_button.winfo_exists():
            self.submit_button.destroy()
        self.submit_button = Button(self.gk_frame, text="Submit",width=8, font=("Comic Sans MS",16), bg="#66E04E", command=self.submit_answer)
        self.submit_button.pack()
    # def start_timer(self):
    #     self.remaining_time = 60
    #     self.update_timer()
    ...
    def submit_answer(self):
        answer_index = self.answer_var.get()
        self.user_answers.append(answer_index)
        if len(self.user_answers) < len(self.gk_questions):
            self.current_question_index += 1
            self.display_question()
        else:
                self.calculate_score()
    ...
    def calculate_score(self):
        ...
        for user_answer, correct_answer in zip(self.user_answers, self.correct_answers):
            if user_answer == correct_answer:
                correct_count += 1
        total_questions = len(self.gk_questions)
        score = (correct_count / total_questions) * 100
        self.show_result(score)
...
#     t.withdraw()
#     tw = Tk()
#     tw.geometry("1600x1500")
#     bw = Button(tw, text="START", font=("copperplate gothic bold", 22), width=10, bg="#DD7BFF", command=strt)
#     bw.place(x=445, y=250)
#     tw.mainloop()
...
l = Label(t, text="CADD CENTRE", font=("Copperplate Gothic Bold", 90), fg="#6308B8", bg="#3FFFF3")
l.place(x=340, y=20)
